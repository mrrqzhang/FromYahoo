...
sub word_align {
   ...
   #build arguments for symal
    ...
    if($insertnull) {
        safesystem("$GIZA2BAL -d $__ALIGNMENT_INV_CMD -i $__ALIGNMENT_CMD > giza2bal.old ") ;
        safesystem("python $INSERTNULL $___CORPUS.$___E $___CORPUS.$___F giza2bal.old $___CORPUS.$___E_PA $___CORPUS.$___F_PA giza2bal.new"); 
        safesystem("cat giza2bal.new |".
...
sub get_lexical_factored {
    ...
    if ($___NOT_FACTORED && !$_XML) {
	&get_lexical($___CORPUS.".".$___F_PA,
		     $___CORPUS.".".$___E_PA,
		     $___ALIGNMENT_FILE.".".$___ALIGNMENT,
		     $___LEXICAL_FILE,
		     $___LEXICAL_COUNTS,
                     $_BASELINE_CORPUS.".".$___F_PA,
                     $_BASELINE_CORPUS.".".$___E_PA,
                     $_BASELINE_ALIGNMENT,
                     $_INSTANCE_WEIGHTS_FILE);
    }
    else {
	foreach my $factor (split(/\+/,$___TRANSLATION_FACTORS)) {
	    ...
	    &reduce_factors($___CORPUS.".".$___F_PA,
			    $___ALIGNMENT_STEM.".".$factor_f.".".$___F,
			    $factor_f);
	    &reduce_factors($___CORPUS.".".$___E_PA,
	    ...
	    &get_lexical($___ALIGNMENT_STEM.".".$factor_f.".".$___F,
			 ...
			 $lexical_file,
			 $___LEXICAL_COUNTS,
                         $_BASELINE_CORPUS.".".$factor_f.".".$___F_PA,
                         $_BASELINE_CORPUS.".".$factor_e.".".$___E_PA,
                         $_BASELINE_ALIGNMENT,
                         $_INSTANCE_WEIGHTS_FILE);
	}
    }
...
sub extract_phrase_factored {
    ...
    if ($___NOT_FACTORED) {
	&extract_phrase($___CORPUS.".".$___F_PA,
			$___CORPUS.".".$___E_PA,
			$___EXTRACT_FILE,
...
sub get_generation_factored {
  ...
  my $corpus = $___CORPUS.".".$___E_PA.$___CORPUS_COMPRESSION;
