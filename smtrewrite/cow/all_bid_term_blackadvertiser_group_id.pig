...
%default arbitriage 'hdfs://jetblue-nn1.blue.ygrid.yahoo.com:8020/user/ruiqiang/data/advertiser-blacklist-binlu.txt'
...
A61 = LOAD '$arbitriage' using PigStorage('\t','-noschema') as (advertiser_id:chararray) ;
...
-- bid
B1 = LOAD '$cowgrid/projects/cow_prod/complete_snapshot/$COW_DATE/adGroup' USING JsonLoader AS (json:map[]);
...
C2 = FOREACH C1 GENERATE A5::phrase_canon AS phrase_canon, D3::bid AS bid, D3::advertiser_id as advertiser_id, D3::ad_group_id as ad_group_id, A5::match_type as match_type;
C3 = GROUP C2 BY (phrase_canon,advertiser_id,ad_group_id,match_type) PARALLEL 400;
C4 = FOREACH C3 {
       S1 = FOREACH C2 GENERATE bid;
       S2 = ORDER S1 BY bid DESC;
       S3 = LIMIT S2 1;
       GENERATE FLATTEN(group) AS (phrase_canon,advertiser_id,ad_group_id,match_type), FLATTEN(S3) AS (bid);
     };
