...
if [ 1 -eq "$(echo "1 >= $start" | bc)" ] && [ 1 -eq "$(echo "$end >= 1" | bc)" ]; then
     ...
     hadoop fs -cat $firstpassroot/decode-edu/postclickrewrite/thresholdcut/part* | python secondpass-select.py > $firstpassqrw
...
if [ 1 -eq "$(echo "2 >= $start" | bc)" ] && [ 1 -eq "$(echo "$end >= 2" | bc)" ]; then
    sh suite-smt-rewrite.sh -m onlinetest -q default -s 0 -e 2.2  -g 300 -n 1 -b 5000 -d $secondpassdecode -l 10 -r $firstpassroot -w /projects/qrw/ruiqiang/all_bid_terms_use_blacklist_binlu/201511160000/normal-advertiser/part*  -i $firstpassbidterms
fi
...
if [ 1 -eq "$(echo "4 >= $start" | bc)" ] && [ 1 -eq "$(echo "$end >= 4" | bc)" ]; then
    sh suite-smt-rewrite.sh -m onlinetest -q default -t $btfilter2 -s 2.4 -e 2.6 -g 300 -n 1 -b 5000 -d $secondpassdecode -l 10 -r $firstpassroot -w /projects/qrw/ruiqiang/all_bid_terms_use_blacklist_binlu/201511160000/normal-advertiser/part*  -i $firstpassbidterms
fi

if [ 1 -eq "$(echo "5 >= $start" | bc)" ] && [ 1 -eq "$(echo "$end >= 5" | bc)" ]; then
   sh dopig.sh secondpassmerge.pig "-Dmapred.output.compress=false -p first=$firstpassroot/decode-edu/postclickrewrite/thresholdcut/part* second=$firstpassroot/$secondpassdecode/postclickrewrite/thresholdcut/part* -p out=$firstpassroot/$secondpassdecode/final"
fi
