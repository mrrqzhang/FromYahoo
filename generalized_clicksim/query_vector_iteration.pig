...
query_url_clicks_tmp0 = load '$input_doc_click' USING PigStorage('\t') AS (f1:chararray,f2:chararray,f3:chararray);
...
B = load '$doc_vector' USING PigStorage('\t') AS (f1:chararray,f2:chararray,f3:chararray);
...
store C3 into '$next_query_vector';
