...
# choose top units
scp gen_top_units_unibigrams.pig $gateway_dir
scp $MLR_SCRIPTS/myudf.py $gateway_dir
ssh $grid 'hadoop fs -rm -r -skipTrash '"$output_hdfs_dir"'/top_units_unibigrams.gz'
ssh $grid 'cd '"$gateway_working_dir"';pig -useversion 0.11 -param output='"$output_hdfs_dir"'/top_units_unibigrams.gz -param gateway_working_dir='"$gateway_working_dir"' -Dmapred.job.queue.name='"$queue"' -Dmapred.reduce.tasks=2000 gen_top_units_unibigrams.pig'

scp gen_top_units_ngrams.pig $gateway_dir
scp $MLR_SCRIPTS/myudf.py $gateway_dir
ssh $grid 'hadoop fs -rm -r -skipTrash '"$output_hdfs_dir"'/top_units_ngrams.gz'
ssh $grid 'cd '"$gateway_working_dir"';pig -useversion 0.11 -param output='"$output_hdfs_dir"'/top_units_ngrams.gz -param gateway_working_dir='"$gateway_working_dir"' -Dmapred.job.queue.name='"$queue"' -Dmapred.reduce.tasks=2000 gen_top_units_ngrams.pig'

#scp gen_top_units_qvec.pig $gateway_dir
#ssh $grid 'hadoop fs -rm -r -skipTrash '"$output_hdfs_dir"'/top_units_qvec.gz'
#ssh $grid 'cd '"$gateway_working_dir"';pig -param output='"$output_hdfs_dir"'/top_units_qvec.gz -param gateway_working_dir='"$gateway_working_dir"' -Dmapred.job.queue.name='"$queue"' -Dmapred.reduce.tasks=2000 gen_top_units_qvec.pig'

# training data generation

# query vectors that do not match any unit
#scp vectors_not_matching_unit.pig $gateway_dir
#ssh $grid 'hadoop fs -rm -r -skipTrash '"$output_hdfs_dir"'/qv_not_matching_unit.gz'
#ssh $grid 'cd '"$gateway_working_dir"';pig -param output='"$output_hdfs_dir"'/qv_not_matching_unit.gz -param input_vector=/projects/clickmodel/ckang/usmlr/sim_v4/query_vector_i2_google_truncated_20.gz -param input1='"$output_hdfs_dir"'/top_units_unibigrams.gz -param input2='"$output_hdfs_dir"'/top_units_qvec.gz -param gateway_working_dir='"$gateway_working_dir"' -Dmapred.job.queue.name='"$queue"' -Dmapred.reduce.tasks=2000 vectors_not_matching_unit.pig'
...
ssh $grid 'cd '"$gateway_working_dir"';pig -useversion 0.11 -param units1='"$output_hdfs_dir"'/top_units_unibigrams.gz -param units2='"$output_hdfs_dir"'/top_units_ngrams.gz -param output='"$output_hdfs_dir"'/queries_decomposed.gz -param gateway_working_dir='"$gateway_working_dir"' -Dmapred.job.queue.name='"$queue"' -Dmapred.reduce.tasks=5000 find_queries_covered_by_units.pig'
...
ssh $grid 'cd '"$gateway_working_dir"';pig -useversion 0.11 -param input_qu_click='"$input_qu_click"' -param input1='"$output_hdfs_dir"'/top_units_unibigrams.gz -param input2='"$output_hdfs_dir"'/top_units_ngrams.gz -param output='"$output_hdfs_dir"'/unit_doc_clicks.gz -param gateway_working_dir='"$gateway_working_dir"' -Dmapred.job.queue.name='"$queue"' -Dmapred.reduce.tasks=8000 gen_unit_vectors.pig'
...
ssh $grid 'cd '"$gateway_working_dir"';pig -useversion 0.11 -param output_hdfs_dir='"$output_hdfs_dir"' -param output='"$output_hdfs_dir"'/training.gz -Dmapred.job.queue.name='"$queue"' -Dmapred.reduce.tasks=5000 gen_training.pig'
...
ssh $grid 'cd '"$gateway_working_dir"';pig -useversion 0.11 -param output_hdfs_dir='"$output_hdfs_dir"' -param output='"$output_hdfs_dir"'/test.gz -Dmapred.job.queue.name='"$queue"' -Dmapred.reduce.tasks=5000 gen_test.pig'
...
ssh $grid 'cd '"$gateway_working_dir"';pig -useversion 0.11 -param output_hdfs_dir='"$output_hdfs_dir"' -param output='"$output_hdfs_dir"'/test_with_weight.gz -Dmapred.job.queue.name='"$queue"' -Dmapred.reduce.tasks=5000 gen_test_with_weight.pig'
