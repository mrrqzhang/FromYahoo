...
DEFINE CMD_NORM_QV `python normalize_vector_demote_top.py 0.00001 10` ship('$gateway_working_dir/normalize_vector_demote_top.py');
DEFINE URL_NORM `python url_norm.py` ship('$gateway_working_dir/url_norm.py');

query_url_clicks_tmp = load '$input_qu_click' USING PigStorage('\t') AS (f1:chararray,f2:chararray,f3:chararray);
...
C5 = order C4 by $0;
store C5 into '$output_hdfs_dir/$modified_doc_vector';
