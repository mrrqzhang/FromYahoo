...
def readQu(quFile):
    ...
    try:
        ...
        print >> sys.stderr, '[ERROR] query is not in %s.'% quFile
        sys.exit()
    try:
        uIdx = heads.index('url')
    except:
        print >> sys.stderr, '[ERROR] url is not in %s.'% quFile
        sys.exit()
...
def groupSameQ(lQuJ, isRandom = True, isSortbyJdg = False):
    ...
    for i,(q,u,g,j) in enumerate(lQuJ):
        ...
        if isRandom and not isSortbyJdg:
            ...
            v += random.random()
        if isSortbyJdg:
            v += 1e-4*DCG.score[j['grade'].lower()[0]]
...
%prog <quj> <score>"""
    ...
    lScore = [float(a.strip()) for a in open(args[2]).readlines() if a.strip()]
